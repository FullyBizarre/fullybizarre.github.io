...
function collison()
{
	...
	//distance > 0 makes it so that rocks that past the player won't have collision while rocks ahead still do, while distance < 4 makes it so that if distance between snake and rock is smaller than 4, code inside will run
	if (distance < 4 && distance > 0)
	{
		// if the y distance between the rock and player is snaller than 0.5, collision detected and game restart
		if(posY < 0.5)
		{
			restart();
		}
